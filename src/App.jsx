...
// Note: auth uses /auth/v1/signup, /auth/v1/token, etc.
...
const DB = {
  ...
  upsert:  (t, data, token) => db(t, { method:"POST", body: JSON.stringify(data), prefer:"resolution=merge-duplicates,return=representation" }, token),
};
...
const fmt = (n, currency = "COP") =>
  new Intl.NumberFormat("es-CO", { style:"currency", currency, minimumFractionDigits:0, maximumFractionDigits:0 }).format(n ?? 0);
...
function Btn({ children, onClick, variant="primary", small=false, full=false, disabled=false }) {
  ...
  return (
    <button disabled={disabled} onClick={onClick}
      style={{ background: disabled?"#d1d5db":bg[variant], color: disabled?"#9ca3af":cl[variant], border:"none", borderRadius:12, padding:small?"7px 14px":"13px 18px", fontSize:small?13:15, fontWeight:700, cursor:disabled?"not-allowed":"pointer", width:full?"100%":"auto", marginBottom:full?10:0 }}>
      {children}
    </button>
  );
}
...
function ProgressBar({ value, max, color="#4f46e5", height=10 }) {
  const pct = Math.min((value/max)*100, 100);
  const c = pct>=90?"#ef4444":pct>=70?"#f59e0b":color;
  return (
    <div style={{ background:"#f3f4f6", borderRadius:99, height, overflow:"hidden" }}>
      <div style={{ background:c, height:"100%", width:${pct}%, borderRadius:99, transition:"width 0.5s" }} />
    </div>
  );
}
...
function Toast({ msg, onDone }) {
  ...
  return (
    <div style={{ position:"fixed", top:60, left:"50%", transform:"translateX(-50%)", background:"#1f2937", color:"#fff", padding:"12px 24px", borderRadius:14, fontSize:14, fontWeight:600, zIndex:999, boxShadow:"0 4px 20px rgba(0,0,0,0.3)", whiteSpace:"nowrap" }}>
      {msg}
    </div>
  );
}
...
function Spinner() {
  return (
    <div style={{ display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", minHeight:"100vh", background:"#f5f5f7", gap:16 }}>
      ...
      <div style={{ color:"#6b7280", fontWeight:600 }}>Cargando...</div>
...
// ─── GOAL CARD ────────────────────────────────────────────────────────────────
function GoalCard({ g, profile, onDelete, onAdd, fmt, fmtDate }) {
  ...
  return (
    <div style={{ background:"#f9fafb", borderRadius:14, padding:14, marginBottom:12 }}>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:8 }}>
        ...
        <button onClick={()=>onDelete(g.id)} style={{ background:"none", border:"none", fontSize:14, cursor:"pointer", opacity:0.5 }}>🗑️</button>
      </div>
      <ProgressBar value={g.saved} max={g.target} color={g.color} height={12} />
      <div style={{ display:"flex", justifyContent:"space-between", marginTop:6, fontSize:12 }}>
        <span style={{ fontWeight:700, color:g.color }}>{fmt(g.saved, profile.currency)} ahorrados</span>
        <span style={{ color:"#9ca3af" }}>de {fmt(g.target, profile.currency)} ({pct.toFixed(0)}%)</span>
      </div>
      {!done && (
  ...
  const handle = async () => {
    ...
    try {
      if (mode === "login") {
        ...
        else setMsg("✅ Revisa tu correo para confirmar tu cuenta, luego inicia sesión.");
      } else {
      ...
      <div style={{ background:"#fff", borderRadius:"28px 28px 0 0", width:"100%", maxWidth:430, padding:"32px 28px 48px" }}>
        <div style={{ textAlign:"center", marginBottom:28 }}>
          ...
          <div style={{ fontSize:14, color:"#9ca3af", marginTop:4 }}>
            {mode==="login"?"Inicia sesión para continuar":mode==="register"?"Crea tu cuenta":"Recuperar contraseña"}
          </div>
        </div>
        ...
        <Field label="Correo electrónico">
          <Inp type="email" placeholder="tucorreo@email.com" value={email} onChange={e=>setEmail(e.target.value)} />
        </Field>

        {mode !== "reset" && (
          <Field label="Contraseña">
            <Inp type="password" placeholder="Mínimo 6 caracteres" value={pass} onChange={e=>setPass(e.target.value)} onKeyDown={e=>e.key==="Enter"&&handle()} />
          </Field>
        )}

        {mode === "register" && (
          <Field label="Confirmar contraseña">
            <Inp type="password" placeholder="Repite tu contraseña" value={pass2} onChange={e=>setPass2(e.target.value)} onKeyDown={e=>e.key==="Enter"&&handle()} />
          </Field>
        )}

        <Btn onClick={handle} variant="primary" full disabled={loading}>
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:10, marginTop:8, alignItems:"center" }}>
          ...
          {mode !== "login" && (
            <button onClick={()=>{setMode("login");setError("");setMsg("");}} style={{ background:"none", border:"none", color:"#4f46e5", fontWeight:700, fontSize:14, cursor:"pointer" }}>← Volver a iniciar sesión</button>
          )}
        </div>
      </div>
    </div>
...
export default function App() {
  ...
  useEffect(() => {
    ...
    if (stored) {
      try { const s = JSON.parse(stored); setSession(s); }
      catch {}
    }
    setAuthReady(true);
  }, []);
  ...
  const loadAll = async () => {
    ...
    try {
      const [tx, ct, bg, gl, db_, pf] = await Promise.all([
        DB.getAll("transactions", token),
        DB.getAll("contacts",     token),
        DB.getAll("budgets",      token),
        DB.getAll("goals",        token),
        DB.getAll("debts",        token),
        DB.getAll("profile",      token),
      ]);
      setTransactions(tx);
      setContacts(ct);
      setBudgets(bg);
    ...
    } catch (e) { notify("⚠️ Error cargando datos: " + e.message); }
    setLoading(false);
  };

  const handleLogin = (data) => {
    localStorage.setItem("fin_session", JSON.stringify(data));
    setSession(data);
  };
  ...
  const handleLogout = async () => {
    ...
    setSession(null);
    setTransactions([]); setContacts([]); setBudgets([]); setGoals([]); setDebts([]);
  ...
  const addTx = async () => {
    ...
    try {
      ...
      setForm({ type:"income", amount:"", category:"Salario", description:"", date:today(), contact:"", note:"" });
      setShowAdd(false);
      notify("✅ Movimiento guardado en la nube");
    } catch(e) { notify("⚠️ Error: "+e.message); }
  };

  const deleteTx = async (id) => {
    try {
      await DB.remove("transactions", id, token);
  ...
  const addBudget = async () => {
    ...
    try {
      const [saved] = await DB.insert("budgets", { category:budgetForm.category, limit:parseFloat(budgetForm.limit), user_id:userId }, token);
      setBudgets(p=>[...p, saved]);
      setBudgetForm({ category:"Comida", limit:"" }); setShowAddBudget(false);
      notify("✅ Presupuesto guardado");
    } catch(e) { notify("⚠️ Error: "+e.message); }
  };
  const deleteBudget = async (id) => {
    await DB.remove("budgets", id, token);
  ...
  const addGoal = async () => {
    ...
    try {
      const [saved] = await DB.insert("goals", { name:goalForm.name, target:parseFloat(goalForm.target), saved:parseFloat(goalForm.saved||0), deadline:goalForm.deadline||null, color:goalForm.color, user_id:userId }, token);
      setGoals(p=>[...p, saved]);
      setGoalForm({ name:"", target:"", saved:"0", deadline:"", color:"#6366f1" }); setShowAddGoal(false);
      notify("🎯 Meta guardada");
    } catch(e) { notify("⚠️ Error: "+e.message); }
  };
  const deleteGoal = async (id) => {
    await DB.remove("goals", id, token);
  ...
  const addToGoal = async (id, amt) => {
    ...
    try {
      await DB.update("goals", id, { saved: newSaved }, token);
      setGoals(p=>p.map(g=>g.id===id?{...g, saved:newSaved}:g));
      notify(💰 ${fmt(amt, profile.currency)} agregados a "${g.name}");
    } catch(e) { notify("⚠️ Error: "+e.message); }
  };
  ...
  const addDebt = async () => {
    ...
    try {
      const [saved] = await DB.insert("debts", { type:debtForm.type, person:debtForm.person, amount:parseFloat(debtForm.amount), reason:debtForm.reason||"", date:debtForm.date, paid:false, user_id:userId }, token);
      setDebts(p=>[...p, saved]);
      setDebtForm({ type:"owed", person:"", amount:"", reason:"", date:today() }); setShowAddDebt(false);
      notify("✅ Deuda registrada");
    } catch(e) { notify("⚠️ Error: "+e.message); }
  };
  const toggleDebt = async (id) => {
    const d = debts.find(d=>d.id===id);
    await DB.update("debts", id, { paid: !d.paid }, token);
    setDebts(p=>p.map(d=>d.id===id?{...d,paid:!d.paid}:d));
    notify("✅ Estado actualizado");
  };
  const deleteDebt = async (id) => {
    await DB.remove("debts", id, token);
  ...
  const addContact = async () => {
    ...
    try {
      const [saved] = await DB.insert("contacts", { name:contactForm.name, phone:contactForm.phone||"", notes:contactForm.notes||"", user_id:userId }, token);
      setContacts(p=>[...p, saved]);
      setContactForm({ name:"", phone:"", notes:"" }); setShowAddContact(false);
  ...
  const saveProfile = async () => {
    try {
      ...
      if (profile.id) {
        await DB.update("profile", profile.id, data, token);
        setProfile({...data, id:profile.id});
      } else {
        const [saved] = await DB.insert("profile", data, token);
        setProfile(saved);
      }
  ...
  const exportExcel = useCallback(() => {
    ...
    const ws1 = XLSX.utils.aoa_to_sheet([
      ["Fecha","Tipo","Categoría","Descripción","Monto","Persona","Nota"],
      ...transactions.sort((a,b)=>b.date.localeCompare(a.date)).map(t=>[fmtDate(t.date), TYPE_LABELS[t.type], t.category, t.description, t.type==="income"?t.amount:-t.amount, t.contact||"", t.note||""])
    ]);
    ws1["!cols"] = [14,10,18,28,16,20,24].map(w=>({wch:w}));
    XLSX.utils.book_append_sheet(wb, ws1, "Movimientos");

    const monthMap = {};
    transactions.forEach(t=>{ const m=t.date.slice(0,7); if(!monthMap[m]) monthMap[m]={income:0,expense:0,cost:0}; monthMap[m][t.type]+=t.amount; });
    const ws2 = XLSX.utils.aoa_to_sheet([
      ["Mes","Ingresos","Gastos","Costos","Balance"],
      ...Object.entries(monthMap).sort((a,b)=>b[0].localeCompare(a[0])).map(([m,v])=>{
        const [y,mo]=m.split("-");
        return [${MONTHS_ES[parseInt(mo)-1]} ${y}, v.income, v.expense, v.cost, v.income-v.expense-v.cost];
      })
    ]);
    XLSX.utils.book_append_sheet(wb, ws2, "Resumen mensual");

    const ws3 = XLSX.utils.aoa_to_sheet([["Categoría","Tipo","Total"], ...byCategory.map(([c,d])=>[c,TYPE_LABELS[d.type],d.total])]);
    XLSX.utils.book_append_sheet(wb, ws3, "Por categoría");

    const ws4 = XLSX.utils.aoa_to_sheet([["Nombre","Teléfono","Notas","Total recibido"], ...contacts.map(c=>[c.name,c.phone||"",c.notes||"",contactTotal(c.name)])]);
    XLSX.utils.book_append_sheet(wb, ws4, "Contactos");

    const ws5 = XLSX.utils.aoa_to_sheet([["Tipo","Persona","Monto","Razón","Fecha","Estado"], ...debts.map(d=>[d.type==="owed"?"Me deben":"Debo",d.person,d.amount,d.reason||"",fmtDate(d.date),d.paid?"Liquidado":"Pendiente"])]);
    XLSX.utils.book_append_sheet(wb, ws5, "Deudas");

    XLSX.writeFile(wb, MisFinanzas_${thisMonth()}.xlsx);
    notify("📊 Excel exportado");
  }, [transactions, contacts, debts, byCategory]);
  ...
  const monthLabel = ${MONTHS_ES[parseInt(mo)-1]} ${y};
  ...
  return (
    <div style={{ fontFamily:"-apple-system,BlinkMacSystemFont,'SF Pro Display',sans-serif", background:"#f5f5f7", minHeight:"100vh", maxWidth:430, margin:"0 auto", position:"relative", paddingBottom:88 }}>
      ...
      <div style={{ background:"linear-gradient(145deg,#4f46e5 0%,#7c3aed 60%,#a855f7 100%)", padding:"52px 20px 28px", color:"#fff", position:"relative" }}>
        <button onClick={()=>{ setProfileForm(profile); setShowProfile(true); }}
          style={{ position:"absolute", top:16, right:16, background:"rgba(255,255,255,0.2)", border:"none", borderRadius:"50%", width:36, height:36, fontSize:18, cursor:"pointer" }}>⚙️</button>
        ...
        <div style={{ fontSize:12, opacity:0.7, marginTop:4 }}>
          {balance>=0 ? ✅ Ahorro del ${savingPct.toFixed(1)}% este mes : ⚠️ Gastas más de lo que ingresas}
        </div>
        {profile.monthly_saving_goal > 0 && (
          <div style={{ marginTop:12 }}>
            <div style={{ fontSize:11, opacity:0.7, marginBottom:4 }}>Meta de ahorro: {fmt(profile.monthly_saving_goal, currency)}</div>
            <ProgressBar value={Math.max(balance,0)} max={profile.monthly_saving_goal} color="#a5f3fc" height={6} />
          </div>
        )}
        ...
        {availMonths.map(m=>{
          ...
          return (
            <button key={m} onClick={()=>setFilterMonth(m)}
              style={{ background:filterMonth===m?"#4f46e5":"#f3f4f6", color:filterMonth===m?"#fff":"#374151", border:"none", borderRadius:20, padding:"6px 14px", fontSize:13, fontWeight:700, whiteSpace:"nowrap", cursor:"pointer", flexShrink:0 }}>
              {MONTHS_ES[parseInt(mm)-1].slice(0,3)} {yy}
            </button>
          );
        })}
      </div>
      ...
      <div style={{ padding:"14px 16px 0" }}>
        ...
        {tab==="dashboard" && (<>
          {budgets.filter(b=>{ const u=budgetUsage(b.category); return u/b.limit>=0.7; }).map(b=>{
            ...
            return (
              <div key={b.id} style={{ background:over?"#fee2e2":"#fef3c7", borderRadius:14, padding:"12px 16px", marginBottom:10, display:"flex", alignItems:"center", gap:10 }}>
                <span style={{ fontSize:22 }}>{over?"🚨":"⚠️"}</span>
                <div>
                  <div style={{ fontWeight:700, fontSize:14, color:over?"#991b1b":"#92400e" }}>{over?Presupuesto de ${b.category} superado:${b.category} al ${pct}% del límite}</div>
                  <div style={{ fontSize:12, color:"#6b7280" }}>{fmt(u, currency)} / {fmt(b.limit, currency)}</div>
                </div>
              </div>
            );
          })}
          ...
          {(debtTotals.owed>0||debtTotals.owing>0) && (
            <div style={{ background:"#fff", borderRadius:16, padding:16, marginBottom:14, boxShadow:"0 1px 6px rgba(0,0,0,0.06)" }}>
              ...
              <div style={{ display:"flex", gap:10 }}>
                <div style={{ flex:1, background:"#d1fae5", borderRadius:12, padding:12 }}>
                  <div style={{ fontSize:11, color:"#065f46", fontWeight:700 }}>ME DEBEN</div>
                  <div style={{ fontWeight:800, color:"#059669", fontSize:18 }}>{fmt(debtTotals.owed, currency)}</div>
                </div>
                <div style={{ flex:1, background:"#fee2e2", borderRadius:12, padding:12 }}>
                  <div style={{ fontSize:11, color:"#991b1b", fontWeight:700 }}>DEBO</div>
                  <div style={{ fontWeight:800, color:"#dc2626", fontSize:18 }}>{fmt(debtTotals.owing, currency)}</div>
                </div>
              </div>
            </div>
          )}
          ...
          <div style={{ background:"#fff", borderRadius:16, overflow:"hidden", boxShadow:"0 1px 6px rgba(0,0,0,0.06)", marginBottom:14 }}>
            ...
            {transactions.length===0 && <div style={{ padding:"16px 16px 20px", color:"#9ca3af", fontSize:14 }}>Aún no tienes movimientos. ¡Agrega el primero!</div>}
            {transactions.slice().sort((a,b)=>new Date(b.date)-new Date(a.date)).slice(0,6).map(t=>(
              <div key={t.id} onClick={()=>setShowDetail(t)} style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"11px 16px", borderTop:"1px solid #f9f9f9", cursor:"pointer" }}>
                <div style={{ display:"flex", alignItems:"center", gap:12 }}>
                  ...
                  <div>
                    <div style={{ fontWeight:700, fontSize:14 }}>{t.description}</div>
                    <div style={{ fontSize:11, color:"#9ca3af" }}>{t.category} · {fmtDate(t.date)}</div>
                  </div>
                </div>
          ...
          <div style={{ background:"#fff", borderRadius:16, padding:16, marginBottom:14, boxShadow:"0 1px 6px rgba(0,0,0,0.06)" }}>
            <div style={{ fontWeight:800, marginBottom:12 }}>💡 Insights del mes</div>
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10 }}>
              {[
                { icon:"🎯", label:"Tasa de ahorro",    val:${savingPct.toFixed(1)}% },
                { icon:"📤", label:"Total egresos",     val:fmt(totals.expense+totals.cost, currency) },
                { icon:"💰", label:"Mayor ingreso",     val:fmt(Math.max(...monthTx.filter(t=>t.type==="income").map(t=>t.amount),0), currency) },
                { icon:"📆", label:"Gasto promedio/día",val:fmt((totals.expense+totals.cost)/30, currency) },
              ].map(s=>(
                <div key={s.label} style={{ background:"#f9fafb", borderRadius:12, padding:"10px 12px" }}>
                  ...
                  <div style={{ fontSize:14, fontWeight:900, color:"#1f2937" }}>{s.val}</div>
                </div>
              ))}
            </div>
          </div>

          <Btn onClick={exportExcel} variant="dark" full>📊 Exportar todo a Excel</Btn>
          <div style={{ marginTop:10 }}>
            <Btn onClick={handleLogout} variant="ghost" full>🚪 Cerrar sesión</Btn>
          </div>
        </>)}
        ...
        {tab==="transactions" && (<>
          <div style={{ position:"relative", marginBottom:12 }}>
            ...
            <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Buscar movimiento..." style={{ ...inputSx, paddingLeft:40 }} />
          </div>
          <div style={{ display:"flex", gap:8, marginBottom:14, overflowX:"auto", scrollbarWidth:"none" }}>
            {["all","income","expense","cost"].map(t=>(
              <button key={t} onClick={()=>setFilterType(t)}
                style={{ background:filterType===t?"#4f46e5":"#f3f4f6", color:filterType===t?"#fff":"#374151", border:"none", borderRadius:20, padding:"7px 14px", fontSize:13, fontWeight:700, whiteSpace:"nowrap", cursor:"pointer", flexShrink:0 }}>
                {t==="all"?"Todos":TYPE_LABELS[t]}
              </button>
            ))}
          </div>
          <div style={{ background:"#fff", borderRadius:16, overflow:"hidden", boxShadow:"0 1px 6px rgba(0,0,0,0.06)" }}>
            {filtered.length===0 && <div style={{ padding:32, textAlign:"center", color:"#9ca3af" }}><div style={{ fontSize:32, marginBottom:8 }}>🔎</div><div style={{ fontWeight:600 }}>Sin resultados</div></div>}
            {filtered.map((t,i)=>(
              <div key={t.id} onClick={()=>setShowDetail(t)} style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"13px 16px", borderTop:i>0?"1px solid #f3f4f6":"none", cursor:"pointer" }}>
                <div style={{ display:"flex", alignItems:"center", gap:12 }}>
                  ...
                  <div>
                    <div style={{ fontWeight:700, fontSize:14 }}>{t.description}</div>
                    <div style={{ fontSize:11, color:"#9ca3af" }}>{t.category} · {fmtDate(t.date)}</div>
                    {t.contact && <div style={{ fontSize:11, color:"#4f46e5", marginTop:1 }}>👤 {t.contact}</div>}
                  </div>
                </div>
        ...
        {tab==="analytics" && (<>
          <div style={{ background:"#fff", borderRadius:16, padding:"16px 16px 20px", marginBottom:14, boxShadow:"0 1px 6px rgba(0,0,0,0.06)" }}>
              ...
              {trend.map((m,i)=>(
                <div key={i} style={{ flex:1, display:"flex", flexDirection:"column", alignItems:"center" }}>
                  <div style={{ width:"100%", display:"flex", gap:2, height:110, alignItems:"flex-end", justifyContent:"center" }}>
                    <div style={{ flex:1, background:"#10b981", borderRadius:"4px 4px 0 0", height:${(m.income/maxTrend)*100}%, minHeight:2, transition:"height 0.5s" }} />
                    <div style={{ flex:1, background:"#ef4444", borderRadius:"4px 4px 0 0", height:${(m.expense/maxTrend)*100}%, minHeight:2, transition:"height 0.5s" }} />
                  </div>
            ...
            <div style={{ display:"flex", gap:16, marginTop:10, justifyContent:"center" }}>
              <div style={{ display:"flex", alignItems:"center", gap:6, fontSize:12, color:"#6b7280" }}><div style={{ width:10,height:10,background:"#10b981",borderRadius:2 }}/>Ingresos</div>
              <div style={{ display:"flex", alignItems:"center", gap:6, fontSize:12, color:"#6b7280" }}><div style={{ width:10,height:10,background:"#ef4444",borderRadius:2 }}/>Egresos</div>
            </div>
          </div>

          <div style={{ background:"#fff", borderRadius:16, padding:16, marginBottom:14, boxShadow:"0 1px 6px rgba(0,0,0,0.06)" }}>
            <div style={{ fontWeight:800, marginBottom:14 }}>🗂️ Por categoría — {monthLabel}</div>
            {byCategory.length===0 && <div style={{ color:"#9ca3af", fontSize:14 }}>Sin movimientos este mes.</div>}
              ...
              return (
                <div key={cat} style={{ marginBottom:14 }}>
                  <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:5 }}>
                    ...
                    <span style={{ fontSize:14, fontWeight:800, color:TYPE_COLORS[data.type] }}>{fmt(data.total, currency)}</span>
                  </div>
                  <ProgressBar value={data.total} max={max} color={TYPE_COLORS[data.type]} />
                </div>
              );
            })}
          </div>
        ...
        {tab==="budgets" && (<>
          <div style={{ background:"#fff", borderRadius:16, padding:16, marginBottom:14, boxShadow:"0 1px 6px rgba(0,0,0,0.06)" }}>
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:14 }}>
              <span style={{ fontWeight:800, fontSize:16 }}>🗓️ Presupuestos</span>
              <Btn onClick={()=>setShowAddBudget(true)} variant="primary" small>+ Agregar</Btn>
            </div>
            {budgets.length===0 && <div style={{ color:"#9ca3af", fontSize:14 }}>Sin presupuestos. Agrégalos para controlar tus gastos.</div>}
            {budgets.map(b=>{
              ...
              return (
                <div key={b.id} style={{ marginBottom:16 }}>
                  <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:6 }}>
                    ...
                    <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                      <span style={{ fontSize:13, fontWeight:700, color:over?"#ef4444":pct>=70?"#f59e0b":"#374151" }}>{fmt(used,currency)} / {fmt(b.limit,currency)}</span>
                      <button onClick={()=>deleteBudget(b.id)} style={{ background:"none", border:"none", fontSize:14, cursor:"pointer", opacity:0.4 }}>🗑️</button>
                    </div>
                  </div>
                  <ProgressBar value={used} max={b.limit} />
                  <div style={{ fontSize:11, color:over?"#ef4444":pct>=70?"#f59e0b":"#9ca3af", marginTop:4, textAlign:"right" }}>
                    {over?Superado por ${fmt(used-b.limit,currency)}:Disponible: ${fmt(b.limit-used,currency)} (${(100-pct).toFixed(0)}%)}
                  </div>
                </div>
              );
            })}
          ...
          <div style={{ background:"#fff", borderRadius:16, padding:16, marginBottom:14, boxShadow:"0 1px 6px rgba(0,0,0,0.06)" }}>
            ...
            {goals.length===0 && <div style={{ color:"#9ca3af", fontSize:14 }}>Sin metas. ¡Define una y empieza a ahorrar!</div>}
            {goals.map(g=>(
              <GoalCard key={g.id} g={g} profile={profile} onDelete={deleteGoal} onAdd={addToGoal} fmt={fmt} fmtDate={fmtDate} />
            ))}
          </div>
          ...
          <div style={{ background:"#fff", borderRadius:16, padding:16, boxShadow:"0 1px 6px rgba(0,0,0,0.06)" }}>
            ...
            {["owed","owing"].map(dtype=>{
              ...
              return (
                <div key={dtype}>
                  <div style={{ fontSize:13, fontWeight:800, color:dtype==="owed"?"#059669":"#dc2626", marginBottom:8, marginTop:4 }}>
                    {dtype==="owed"?"🤝 Me deben":"⚠️ Yo debo"}
                  </div>
                  {list.map(d=>(
                    <div key={d.id} style={{ background:d.paid?"#f9fafb":dtype==="owed"?"#f0fdf4":"#fff1f2", borderRadius:12, padding:12, marginBottom:10, opacity:d.paid?0.6:1 }}>
                      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start" }}>
                        ...
                        <div style={{ textAlign:"right" }}>
                          <div style={{ fontWeight:800, color:dtype==="owed"?"#059669":"#dc2626", fontSize:16 }}>{fmt(d.amount,currency)}</div>
                          <div style={{ fontSize:11, color:d.paid?"#059669":"#f59e0b", fontWeight:700 }}>{d.paid?"✅ Liquidado":"🕐 Pendiente"}</div>
                        </div>
                      </div>
                      <div style={{ display:"flex", gap:8, marginTop:10 }}>
                        <button onClick={()=>toggleDebt(d.id)} style={{ flex:1, background:d.paid?"#f3f4f6":"#d1fae5", color:d.paid?"#6b7280":"#059669", border:"none", borderRadius:10, padding:"7px 0", fontSize:13, fontWeight:700, cursor:"pointer" }}>
                          {d.paid?"↩️ Reabrir":"✅ Liquidar"}
                        </button>
                        <button onClick={()=>deleteDebt(d.id)} style={{ background:"#fee2e2", color:"#dc2626", border:"none", borderRadius:10, padding:"7px 14px", fontSize:13, fontWeight:700, cursor:"pointer" }}>🗑️</button>
                      </div>
                    </div>
                  ))}
                </div>
        ...
        {tab==="contacts" && (<>
          ...
          {contacts.map(c=>{
            ...
            const txCount=transactions.filter(t=>t.contact===c.name).length;
            return (
              <div key={c.id} onClick={()=>setShowContactDetail(c)} style={{ background:"#fff", borderRadius:16, padding:16, marginBottom:12, boxShadow:"0 1px 6px rgba(0,0,0,0.06)", cursor:"pointer", display:"flex", alignItems:"center", gap:14 }}>
                <div style={{ width:50, height:50, borderRadius:"50%", background:"linear-gradient(135deg,#4f46e5,#7c3aed)", display:"flex", alignItems:"center", justifyContent:"center", color:"#fff", fontWeight:800, fontSize:20, flexShrink:0 }}>
                  {c.name.charAt(0).toUpperCase()}
                </div>
                <div style={{ flex:1, minWidth:0 }}>
                  <div style={{ fontWeight:800, fontSize:15 }}>{c.name}</div>
                  {c.phone && <div style={{ fontSize:12, color:"#9ca3af" }}>📞 {c.phone}</div>}
                  {c.notes && <div style={{ fontSize:12, color:"#6b7280", overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{c.notes}</div>}
                </div>
                <div style={{ textAlign:"right", flexShrink:0 }}>
                  <div style={{ fontWeight:800, color:"#10b981", fontSize:14 }}>{fmt(total,currency)}</div>
                  <div style={{ fontSize:11, color:"#9ca3af" }}>{txCount} movimientos</div>
                </div>
              </div>
            );
          })}
      ...
      <div style={{ position:"fixed", bottom:0, left:"50%", transform:"translateX(-50%)", width:"100%", maxWidth:430, background:"#fff", borderTop:"1px solid #f0f0f0", display:"flex", padding:"8px 0 22px", zIndex:100 }}>
        {[
          ...
          { id:"budgets",      icon:"🎯", label:"Metas"      },
          { id:"contacts",     icon:"👥", label:"Personas"   },
        ].map(n=>(
          <button key={n.id} onClick={()=>setTab(n.id)}
            style={{ flex:1, background:"none", border:"none", display:"flex", flexDirection:"column", alignItems:"center", gap:2, cursor:"pointer", padding:"4px 0", color:tab===n.id?"#4f46e5":"#9ca3af" }}>
      ...
      <button onClick={()=>setShowAdd(true)}
        style={{ position:"fixed", bottom:92, right:"calc(50% - 200px)", background:"linear-gradient(135deg,#4f46e5,#7c3aed)", color:"#fff", border:"none", borderRadius:"50%", width:58, height:58, fontSize:30, cursor:"pointer", boxShadow:"0 6px 20px rgba(79,70,229,0.45)", zIndex:110, lineHeight:1 }}>+</button>
        ...
        <Modal title="Nuevo movimiento" onClose={()=>setShowAdd(false)}>
          <Field label="Tipo">
            <div style={{ display:"flex", gap:8, marginBottom:4 }}>
              {["income","expense","cost"].map(t=>(
                <button key={t} onClick={()=>setForm({...form,type:t,category:CATEGORIES[t][0]})}
                  style={{ flex:1, background:form.type===t?${TYPE_COLORS[t]}20:"#f3f4f6", color:form.type===t?TYPE_COLORS[t]:"#374151", border:form.type===t?2px solid ${TYPE_COLORS[t]}:"2px solid transparent", borderRadius:12, padding:"10px 0", fontSize:13, fontWeight:700, cursor:"pointer" }}>
                  {TYPE_ICONS[t]} {TYPE_LABELS[t]}
                </button>
              ))}
            </div>
          </Field>
          <Field label="Monto (COP)"><Inp type="number" placeholder="0" value={form.amount} onChange={e=>setForm({...form,amount:e.target.value})} /></Field>
          ...
          <Field label="Nota adicional (opcional)"><Textarea placeholder="Detalles extras..." value={form.note} onChange={e=>setForm({...form,note:e.target.value})} /></Field>
          {form.type==="income" && (
            <Field label="Persona (opcional)">
              <Sel value={form.contact} onChange={e=>setForm({...form,contact:e.target.value})}>
      ...
      {showDetail && (
        <Modal title="Detalle" onClose={()=>setShowDetail(null)}>
          <div style={{ textAlign:"center", marginBottom:24 }}>
            <div style={{ fontSize:52, marginBottom:8 }}>{TYPE_ICONS[showDetail.type]}</div>
            <div style={{ fontSize:34, fontWeight:900, color:TYPE_COLORS[showDetail.type] }}>{fmt(showDetail.amount,currency)}</div>
            <Badge type={showDetail.type} />
          </div>
          {[["Descripción",showDetail.description],["Categoría",showDetail.category],["Fecha",fmtDate(showDetail.date)],showDetail.contact?["Persona",👤 ${showDetail.contact}]:null,showDetail.note?["Nota",showDetail.note]:null].filter(Boolean).map(([k,v])=>(
          ...
          <div style={{ marginTop:20 }}><Btn onClick={()=>deleteTx(showDetail.id)} variant="danger" full>🗑️ Eliminar movimiento</Btn></div>
        </Modal>
      )}

      {showAddBudget && (
        <Modal title="Nuevo presupuesto" onClose={()=>setShowAddBudget(false)}>
          <Field label="Categoría de gasto"><Sel value={budgetForm.category} onChange={e=>setBudgetForm({...budgetForm,category:e.target.value})}>{[...CATEGORIES.expense,...CATEGORIES.cost].map(c=><option key={c}>{c}</option>)}</Sel></Field>
          <Field label="Límite mensual (COP)"><Inp type="number" placeholder="0" value={budgetForm.limit} onChange={e=>setBudgetForm({...budgetForm,limit:e.target.value})} /></Field>
          <Btn onClick={addBudget} variant="primary" full disabled={!budgetForm.limit}>Guardar presupuesto</Btn>
        </Modal>
      )}

      {showAddGoal && (
        <Modal title="Nueva meta de ahorro" onClose={()=>setShowAddGoal(false)}>
          <Field label="Nombre de la meta"><Inp placeholder="Ej. Fondo de emergencia" value={goalForm.name} onChange={e=>setGoalForm({...goalForm,name:e.target.value})} /></Field>
          <Field label="Meta total (COP)"><Inp type="number" placeholder="0" value={goalForm.target} onChange={e=>setGoalForm({...goalForm,target:e.target.value})} /></Field>
          <Field label="Ya tengo ahorrado (COP)"><Inp type="number" placeholder="0" value={goalForm.saved} onChange={e=>setGoalForm({...goalForm,saved:e.target.value})} /></Field>
          ...
          <Field label="Color">
            <div style={{ display:"flex", gap:10 }}>
              {["#6366f1","#10b981","#f59e0b","#ef4444","#8b5cf6","#06b6d4"].map(c=>(
                <div key={c} onClick={()=>setGoalForm({...goalForm,color:c})}
                  style={{ width:32, height:32, borderRadius:"50%", background:c, cursor:"pointer", border:goalForm.color===c?"3px solid #1f2937":"3px solid transparent" }} />
              ))}
            </div>
          </Field>
      ...
      {showAddDebt && (
        <Modal title="Registrar deuda/préstamo" onClose={()=>setShowAddDebt(false)}>
          <Field label="¿Quién debe?">
            <div style={{ display:"flex", gap:8 }}>
              {["owed","owing"].map(t=>(
                <button key={t} onClick={()=>setDebtForm({...debtForm,type:t})}
                  style={{ flex:1, background:debtForm.type===t?(t==="owed"?"#d1fae5":"#fee2e2"):"#f3f4f6", color:debtForm.type===t?(t==="owed"?"#059669":"#dc2626"):"#374151", border:"none", borderRadius:12, padding:"11px 0", fontSize:14, fontWeight:700, cursor:"pointer" }}>
                  {t==="owed"?"🤝 Me deben":"⚠️ Yo debo"}
                </button>
              ))}
            </div>
          </Field>
          ...
          <Field label="Monto (COP)"><Inp type="number" placeholder="0" value={debtForm.amount} onChange={e=>setDebtForm({...debtForm,amount:e.target.value})} /></Field>
      ...
      {showAddContact && (
        <Modal title="Nueva persona" onClose={()=>setShowAddContact(false)}>
          ...
          <Field label="Notas"><Textarea placeholder="¿Quién es? ¿Para qué?" value={contactForm.notes} onChange={e=>setContactForm({...contactForm,notes:e.target.value})} /></Field>
          <Btn onClick={addContact} variant="primary" full disabled={!contactForm.name}>Guardar persona</Btn>
        </Modal>
      )}
      ...
      {showContactDetail && (
        <Modal title={showContactDetail.name} onClose={()=>setShowContactDetail(null)}>
          <div style={{ textAlign:"center", marginBottom:20 }}>
            <div style={{ width:72, height:72, borderRadius:"50%", background:"linear-gradient(135deg,#4f46e5,#7c3aed)", display:"flex", alignItems:"center", justifyContent:"center", color:"#fff", fontWeight:900, fontSize:28, margin:"0 auto 12px" }}>
              {showContactDetail.name.charAt(0).toUpperCase()}
            </div>
            <div style={{ fontWeight:800, fontSize:20 }}>{showContactDetail.name}</div>
            {showContactDetail.phone && <div style={{ color:"#9ca3af" }}>📞 {showContactDetail.phone}</div>}
          </div>
          <div style={{ background:"#f0fdf4", borderRadius:14, padding:14, marginBottom:16, textAlign:"center" }}>
            <div style={{ fontSize:11, color:"#059669", fontWeight:700, marginBottom:2 }}>TOTAL RECIBIDO</div>
            <div style={{ fontSize:30, fontWeight:900, color:"#059669" }}>{fmt(contactTotal(showContactDetail.name),currency)}</div>
          </div>
          ...
          {transactions.filter(t=>t.contact===showContactDetail.name).length===0
            ? <div style={{ color:"#9ca3af", fontSize:14 }}>Sin movimientos aún.</div>
            : transactions.filter(t=>t.contact===showContactDetail.name).sort((a,b)=>new Date(b.date)-new Date(a.date)).map(t=>(
              <div key={t.id} style={{ display:"flex", justifyContent:"space-between", padding:"11px 0", borderBottom:"1px solid #f3f4f6" }}>
                <div>
                  <div style={{ fontWeight:700, fontSize:14 }}>{t.description}</div>
                  <div style={{ fontSize:12, color:"#9ca3af" }}>{t.category} · {fmtDate(t.date)}</div>
                </div>
                <div style={{ fontWeight:800, color:"#10b981" }}>+{fmt(t.amount,currency)}</div>
              </div>
            ))
          }
        </Modal>
      ...
      {showProfile && (
        <Modal title="Mi perfil" onClose={()=>setShowProfile(false)}>
          ...
          <Field label="Moneda">
            <Sel value={profileForm.currency} onChange={e=>setProfileForm({...profileForm,currency:e.target.value})}>
              {["COP","USD","EUR","MXN","ARS","CLP","PEN","BRL"].map(c=><option key={c}>{c}</option>)}
            </Sel>
          </Field>
          <Field label="Meta de ahorro mensual (COP)"><Inp type="number" placeholder="0" value={profileForm.monthly_saving_goal} onChange={e=>setProfileForm({...profileForm,monthly_saving_goal:e.target.value})} /></Field>
